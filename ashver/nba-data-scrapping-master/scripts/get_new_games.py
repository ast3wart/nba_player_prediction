...
def main():
    ...
    try:
        old_games = pd.read_csv('games.csv')
    except:
        raise Exception(
    ...
    # Load old datasets
    old_ranking = pd.read_csv('ranking.csv')
    old_games_details = pd.read_csv('games_details.csv')
    ...
    if os.path.exists(save_details_path):
        print(f"Loading data from {save_details_path}")
    ...
    for game_id in new_games['GAME_ID']:
        ...
        print(f"Saving data to {save_details_path}")
